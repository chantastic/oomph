defmodule OomphWeb.WeeklyRecurringDependentTaskController do
  ...
  def show(conn, %{"id" => id}) do
    ...
    render(conn, :show, weekly_recurring_dependent_task: weekly_recurring_dependent_task)
  end

  def edit(conn, %{"id" => id}) do
    weekly_recurring_dependent_task = Tasks.get_weekly_recurring_dependent_task!(id)
    changeset = Tasks.change_weekly_recurring_dependent_task(weekly_recurring_dependent_task)
    render(conn, :edit, weekly_recurring_dependent_task: weekly_recurring_dependent_task, changeset: changeset)
  end

  def update(conn, %{"id" => id, "weekly_recurring_dependent_task" => weekly_recurring_dependent_task_params}) do
    weekly_recurring_dependent_task = Tasks.get_weekly_recurring_dependent_task!(id)

    case Tasks.update_weekly_recurring_dependent_task(weekly_recurring_dependent_task, weekly_recurring_dependent_task_params) do
      {:ok, weekly_recurring_dependent_task} ->
        conn
  ...
  def delete(conn, %{"id" => id}) do
    weekly_recurring_dependent_task = Tasks.get_weekly_recurring_dependent_task!(id)
    {:ok, _weekly_recurring_dependent_task} = Tasks.delete_weekly_recurring_dependent_task(weekly_recurring_dependent_task)
